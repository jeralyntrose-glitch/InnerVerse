    ...
    def __init__(self):
        ...
        if self.is_replit:
            try:
                ...
                # Add small delay to let network initialize
                import time
                time.sleep(2)  # Give network 2 seconds to be ready
                ...
                try:
                    ...
                    print(f"[TrainingPairStorage] 🔍 Full error: {test_error}")
                    print(f"[TrainingPairStorage] 🔍 Traceback:")
                    traceback.print_exc()
                    ...
                    if "no default bucket" in error_msg or "defaultbucketerror" in error_type.lower():
                        ...
                        # Test 2: Try a small write to confirm write access (WITH RETRIES)
                        max_write_retries = 3
                        write_success = False
                        
                        for write_attempt in range(max_write_retries):
                            try:
                                print(f"[TrainingPairStorage] 🔍 Testing write access (attempt {write_attempt + 1}/{max_write_retries})...")
                                test_content = f"connectivity_test_{time.time()}"
                                ...
                                write_error_type = type(write_error).__name__
                                print(f"[TrainingPairStorage] ⚠️ Write attempt {write_attempt + 1} failed: {write_error_type}: {write_error}")
                                
                                if write_attempt < max_write_retries - 1:
                                    wait = (write_attempt + 1) * 3  # 3, 6 seconds
                                    print(f"[TrainingPairStorage] 🔄 Retrying write in {wait}s...")
                                    time.sleep(wait)
                                else:
                                    # All retries failed - log full details
                                    print(f"[TrainingPairStorage] ❌ Write test FAILED after {max_write_retries} attempts")
                                    print(f"[TrainingPairStorage] ❌ Final error: {write_error_type}: {write_error}")
                                    traceback.print_exc()
                        ...
                        if write_success:
                            ...
                            print("[TrainingPairStorage] ⚠️ Object Storage has connection issues - will retry on each write")
                            # STILL ENABLE IT - we'll retry on actual writes
                            self.use_object_storage = True
                    elif "connection" in error_type.lower() or "connection" in error_msg:
                        # ConnectionError - might be transient, try retries
                        print(f"[TrainingPairStorage] ⚠️ ConnectionError detected - trying retries...")
                        max_retries = 3
                        connection_ok = False
                        
                        for retry in range(max_retries):
                            try:
                                wait = (retry + 1) * 3  # 3, 6, 9 seconds
                                if retry > 0:
                                    print(f"[TrainingPairStorage] 🔄 Retry {retry}/{max_retries} after {wait}s...")
                                    time.sleep(wait)
                                
                                # Try write test directly (skip read test)
                                test_content = f"connectivity_test_{time.time()}"
                                self.object_storage_client.upload_from_text("_write_test", test_content)
                                print("[TrainingPairStorage] ✅ Connection recovered! Write test passed!")
                                connection_ok = True
                                ...
                                if retry == max_retries - 1:
                                    print(f"[TrainingPairStorage] ❌ All connection retries failed")
                                    traceback.print_exc()
                        
                        if connection_ok:
                            self.use_object_storage = True
                            print("[TrainingPairStorage] ✅ Object Storage OPERATIONAL (after retry)")
                        else:
                            # Still enable it - we'll retry on actual operations
                            self.use_object_storage = True
                            print("[TrainingPairStorage] ⚠️ Enabling Object Storage with retry-on-write strategy")
                    else:
                        # Some other error
                        print(f"[TrainingPairStorage] ⚠️ Unexpected error: {error_type}: {test_error}")
                        traceback.print_exc()
                        # Still try to use it - might work on actual operations
                        self.use_object_storage = True
                        print("[TrainingPairStorage] ⚠️ Enabling Object Storage (will retry on errors)")
...
# Define fallback storage class (used if initialization fails)
...
# Simplified approach: Development uses local storage only, Production tries Object Storage
is_replit_env = bool(
    os.environ.get('REPL_ID') or 
    os.environ.get('REPLIT') or 
    os.environ.get('REPL_SLUG')
...
if not is_replit_env:
    # Development: Use simple local storage only (no Object Storage complexity)
    print("[TrainingPairStorage] 📁 Development mode detected - using local storage only")
    try:
        training_storage = TrainingPairStorage()
        # Force local storage in development (skip Object Storage entirely)
        training_storage.use_object_storage = False
        training_storage.ensure_directories()
        print("[TrainingPairStorage] ✅ Local storage initialized for development")
    except Exception as e:
        import traceback
        print(f"⚠️ [TrainingPairStorage] Development init failed: {e}")
        traceback.print_exc()
        # Fallback to minimal local storage
        training_storage = FallbackTrainingStorage()
        print("[TrainingPairStorage] ✅ Using fallback local storage for development")
else:
    # Production (Replit): Try Object Storage with clean fallback
    print("[TrainingPairStorage] 🔍 Production mode detected - attempting Object Storage")
    try:
        training_storage = TrainingPairStorage()
        print(f"[TrainingPairStorage] ✅ Initialized (Object Storage: {training_storage.use_object_storage})")
    except Exception as e:
        # Production fallback: If Object Storage init fails, use local storage
        import traceback
        print(f"⚠️ ═════════════════════════════════════════════════════════════")
        print(f"⚠️ [PRODUCTION] TrainingPairStorage initialization failed!")
        print(f"⚠️ Error: {type(e).__name__}: {e}")
        print(f"⚠️ Falling back to local storage (data will NOT persist across deploys)")
        print(f"⚠️ ═════════════════════════════════════════════════════════════")
        traceback.print_exc()
        
        # Create fallback for production
        training_storage = FallbackTrainingStorage()
        print("[TrainingPairStorage] ✅ Using fallback local storage for production")
...
def generate_qa_pairs_sonnet(chunk: str) -> list[dict]:
    ...
    try:
        ...
        log_api_usage("training_pair_generation", "claude-sonnet-4-5", input_tokens, output_tokens, cost)
...
async def send_message_streaming(conversation_id: int, request: Request):
    ...
    try:
        ...
        # Simple sync generator - FastAPI StreamingResponse handles this natively
        # REVERTED from complex queue-based approach which wasn't working
        def generate():
