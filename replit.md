...
- **InnerVerse Chat** (`/chat`): Beautiful, mobile-optimized chat interface with purple gradient design, ChatGPT-style bubble messages, typing indicators, smooth animations, and auto-scrolling. Built with vanilla JavaScript for maximum compatibility. Connects to existing Claude + Pinecone backend via `/api/claude/chat` endpoint. Perfect for quick conversations on desktop and mobile.
