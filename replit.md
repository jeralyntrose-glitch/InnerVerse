...
- **Key UI Components**:
    ...
    - **Chat Interface**: Claude.ai/ChatGPT-inspired with collapsible sidebar, search, history, real-time streaming, multi-image upload (Claude Sonnet 4 Vision), Markdown rendering (`marked.js`), XSS protection (DOMPurify), and dark/light mode. **Phase 1 Fixes (Nov 2025)**: Fixed input bar to bottom without keyboard jumping (mobile safe-area support), removed teal duplicate question boxes via CSS, implemented race-safe conversation loading with load token system for instant feedback, auto-title generation from first message, and collapsible sidebar with desktop toggle + 80% mobile overlay. **PATCH 13 Status (Nov 19, 2025)**: Reverted timeout-based scrolling approach. Comprehensive fix guide created (`COMPREHENSIVE_FIX_GUIDE.md`) for event-driven scroll management and platform-aware Enter key handling. User's team will implement.
