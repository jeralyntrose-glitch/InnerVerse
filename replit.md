...
- **Claude Master Interface** (`/claude`): Advanced conversational workspace with 7 organized project categories (💕 Relationship Lab, 🎓 MBTI Academy, 🔍 Type Detective, 📈 Trading Psychology, 💼 PM Playbook, ⚡ Quick Hits, 🧠 Brain Dump). Each project contains multiple persistent conversations stored in PostgreSQL. Features automatic InnerVerse backend queries via Claude API with function calling, conversation history, rename/delete functionality, global search across all conversations, and PWA support for mobile installation. Designed for deep, structured MBTI learning sessions with CS Joseph's teaching style built into the system prompts.
